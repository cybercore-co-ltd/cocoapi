...
class YTVOSeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm'):
        ...
        self.evalVids = defaultdict(list)
    ...
    def _prepare(self):
        ...
        if p.useCats:
            gts = self.cocoGt.loadAnns(self.cocoGt.getAnnIds(
                vidIds=p.vidIds, catIds=p.catIds))
        ...
        def iou_seq(d_seq, g_seq):
            ...
            for d, g in zip(d_seq, g_seq):
                if d and g:
                    ...
                    u += maskUtils.area(d)
            if not u > .0:
                print("Mask sizes in video {} and category {} may not match!".format(
                    vidId, catId))
    ...
    def evaluateVid(self, vidId, catId, aRng, maxDet):
        ...
        a = np.array([d['avg_area'] < aRng[0] or d['avg_area'] > aRng[1]
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.iouThrs = np.linspace(.5, 0.95, np.round(
            (0.95 - .5) / .05).astype(int) + 1, endpoint=True)
        self.recThrs = np.linspace(.0, 1.00, np.round(
            (1.00 - .0) / .01).astype(int) + 1, endpoint=True)
        self.maxDets = [1, 10, 100]
        self.areaRng = [[0 ** 2, 1e5 ** 2], [0 ** 2, 128 ** 2],
                        [128 ** 2, 256 ** 2], [256 ** 2, 1e5 ** 2]]
